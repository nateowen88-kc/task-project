...
import { CaptureSourceType, CaptureStatus, PrismaClient, RecurrenceRule, TaskStatus } from "@prisma/client";
...
const reverseCaptureSourceMap = {
    SLACK: "slack",
    EMAIL: "email",
};
const reverseCaptureStatusMap = {
    NEW: "new",
    ACCEPTED: "accepted",
    DISCARDED: "discarded",
};
...
function normalizeRecurrence(isRecurring, recurrenceRule) {
    if (!isRecurring || !recurrenceRule || recurrenceRule === "none") {
        return null;
    }
    return recurrenceMap[recurrenceRule];
}
function normalizeCaptureLinks(item) {
    return [item.sourceUrl].filter((value) => Boolean(value?.trim())).map((value) => value.trim());
}
function getSlackDeepLink(teamId, channelId, messageTs) {
...
function parseSlackFormPayload(rawBody) {
...
function toApiTodayOccurrence(occurrence) {
    return {
        id: occurrence.id,
...
function toApiCapturedItem(item) {
    return {
        id: item.id,
...
async function getNextSortOrder(status) {
...
function isValidCaptureSource(value) {
...
function validateCaptureInput(input) {
    return (typeof input.sourceType === "string" &&
        isValidCaptureSource(input.sourceType) &&
        typeof input.title === "string" &&
        input.title.trim().length > 0);
}
function recursOnDate(task, target) {
...
async function generateRecurringOccurrences(target) {
...
async function generateDailyAgenda(target) {
...
app.post("/api/agenda/generate", async (request, response) => {
    const targetDate = typeof request.body?.date === "string"
        ? request.body.date
        : formatDate(new Date());
    const agenda = await generateDailyAgenda(toDateOnly(targetDate));
    response.json(agenda);
...
app.put("/api/tasks/:id", async (request, response) => {
    ...
    const status = statusMap[input.status];
    const links = normalizeLinks(input.links);
    const sortOrder = existing.status === status ? existing.sortOrder : await getNextSortOrder(status);
