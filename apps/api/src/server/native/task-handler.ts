...
export default async function taskHandler(request: NativeRequest, response: NativeResponse) {
  ...
  if (method === "GET" && pathname === API_ROUTES.tasks.templates) {
    ...
    sendJson(response, 200, templates.map((template) => toApiTaskTemplate(template)));
    return;
  }

  if (method === "POST" && pathname === API_ROUTES.tasks.templates) {
    if (!isWorkspaceAdmin(auth)) {
      sendJson(response, 403, { error: "Only workspace admins can manage templates." });
      return;
    }

    const input = (await readJsonBody<Partial<TaskTemplateInput>>(request)) ?? {};
    if (!validateTaskTemplateInput(input)) {
      sendJson(response, 400, { error: "Invalid template payload." });
      return;
    }
    ...
    sendJson(response, 201, toApiTaskTemplate(template));
    return;
  }

  const templateMatch = matchPath(pathname, "/api/tasks/templates/:id");
  ...
  if (method === "GET" && pathname === API_ROUTES.tasks.playbooks) {
    ...
    sendJson(response, 200, playbooks.map((playbook) => toApiTaskPlaybook(playbook)));
    return;
  }

  if (method === "POST" && pathname === API_ROUTES.tasks.playbooks) {
    if (!isWorkspaceAdmin(auth)) {
      sendJson(response, 403, { error: "Only workspace admins can manage playbooks." });
      return;
    }

    const input = (await readJsonBody<Partial<TaskPlaybookInput>>(request)) ?? {};
    if (!validateTaskPlaybookInput(input)) {
      sendJson(response, 400, { error: "Invalid playbook payload." });
      return;
    }
    ...
    sendJson(response, 201, toApiTaskPlaybook(playbook));
    return;
  }

  const playbookMatch = matchPath(pathname, "/api/tasks/playbooks/:id");
  if (method === "DELETE" && playbookMatch) {
    if (!isWorkspaceAdmin(auth)) {
      sendJson(response, 403, { error: "Only workspace admins can manage playbooks." });
      return;
    }
    ...
    if (!existing) {
      sendJson(response, 404, { error: "Playbook not found." });
      return;
    }
  ...
  const runPlaybookMatch = matchPath(pathname, "/api/tasks/playbooks/:id/run");
  if (method === "POST" && runPlaybookMatch) {
